...
def change_client(conn, client_id, first_name=None, last_name=None, email=None, phones=None):
    cur = conn.cursor()
    cur.execute("""
    UPDATE customer SET first_name=%s, last_name=%s, email=%s WHERE client_id=%s;
...
def find_client(conn, first_name=None, last_name=None, email=None, phone=None):
    cur = conn.cursor()
    cur.execute("""
    SELECT * FROM customer c JOIN phones p ON c.client_id = p.client_id WHERE first_name=%s OR last_name=%s OR 
    email=%s OR phone=%S;
...
with psycopg2.connect(database="clients_db", user="postgres", password="123456") as conn:
    ...
    change_client(conn, 1, first_name='Anna', last_name='Novicova', email='anna.petrova@mail.ru', phones='+79034321233')
    delete_phone(conn, 1)
